      ...
      <main className="mx-auto flex w-full max-w-5xl flex-col gap-8 px-6 pb-20">
        ...
        <section className="grid gap-6 md:grid-cols-2">
          <div className="rounded-3xl border border-black/10 bg-white/60 p-6 backdrop-blur">
            ...
            <ol className="mt-3 space-y-2 text-sm text-[var(--muted)]">
              <li>1) Register your item in the registry.</li>
              <li>2) Add a hidden tag/secret if you can (QR, sticker, sleeve tag).</li>
              <li>3) Set a reward for the finder (optional).</li>
              <li>4) When it’s found, you can release the reward safely.</li>
            </ol>
          </div>
          <div className="rounded-3xl border border-black/10 bg-white/60 p-6 backdrop-blur">
            <h2 className="text-xl font-semibold">If you found something</h2>
            <ol className="mt-3 space-y-2 text-sm text-[var(--muted)]">
              <li>1) Enter the hidden tag/secret if it exists.</li>
              <li>2) Prove you saw it without revealing the secret.</li>
              <li>3) Claim the reward and return the item.</li>
            </ol>
          </div>
        </section>
        ...
        <section className="rounded-3xl border border-black/10 bg-white/60 p-6 backdrop-blur">
          ...
          <p className="mt-2 text-sm text-[var(--muted)]">
            ...
            (or that a finder saw the hidden tag). This keeps the registry public and safe to use.
          </p>
        </section>
        ...
        <section className="rounded-3xl border border-black/10 bg-white/60 p-6 backdrop-blur">
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            <div className="rounded-2xl border border-black/10 bg-white/80 p-4">
              <h3 className="text-sm font-semibold">Fast claim (hidden tag)</h3>
              <p className="mt-2 text-sm text-[var(--muted)]">
                Best for MacBooks, phones, cards. A hidden tag makes the claim instant and fully
                trustless.
              </p>
            </div>
            <div className="rounded-2xl border border-black/10 bg-white/80 p-4">
              <h3 className="text-sm font-semibold">Owner‑verified (no tag)</h3>
              <p className="mt-2 text-sm text-[var(--muted)]">
                For items without a tag. The finder answers a few hints; you confirm and release the
                reward.
              </p>
            </div>
          </div>
